...
/*   Updated: 2024/05/27 14:34:09 by makoch-l         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(void)
{
	...
	map->win_ptr = mlx_new_window(map->mlx_ptr, 100, 100, "fdf");
	
	map->img = mlx_new_image(map->mlx_ptr, 100, 100);
	map->addr = mlx_get_data_addr(map->img, &map->bits_per_pixel, &map->line_length, &map->endian);
	my_mlx_pixel_put(map, 50, 50, WHITE);
